Fork for: Unfolded Self-Reconstruction Hashing
==============================

This repository is forked from https://github.com/erikbern/ann-benchmarks/ to reproduce our paper: [Unfolded Self-Reconstruction LSH: Towards
Machine Unlearning in Approximate Nearest
Neighbour Search]. Note that we modified the original code to add deletion time to `runner.py`. The algorithm parameters for each experiments are in `algos-definations`.

To reproduce the paper:
1. First, install neccessary package: `pip install -r requirements.txt`
2. To run the experiments, execute `bash run.sh`, it will also produce the plots for experiment 1, store at `plots/`
3. To produce the plots for experiment 2 and experiment 3, execute notebook `plot.ipynb`
