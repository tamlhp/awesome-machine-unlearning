...
def get_all_k_recalls_values(dataset_distances, run_distances, at, metrics, epsilon=1e-3):

    if 'all-k-recalls' not in metrics:
        print('Computing all K`s recalls ')
        knn_metrics = metrics.create_group('all-k-recalls')
        queries = run_distances.shape[0]
        ks = run_distances.shape[1]
        
        recalls = np.zeros((ks,queries))
        dataset_distances_arr = np.array(dataset_distances)
        for k in range(ks):
            td = dataset_distances_arr[:,at - 1] + epsilon
            true_positive = np.count_nonzero(run_distances[:,:k+1] <= td[:,None], axis=-1)
            recalls[k] = true_positive / at
        knn_metrics['all-k-recalls'] = recalls.mean(-1)
    else:
        print("Found cached result")
    
    return metrics['all-k-recalls']


def get_all_k_precisions_values(dataset_distances, run_distances, at, metrics, epsilon=1e-3):

    if 'all-k-precisions' not in metrics:
        print('Computing all K`s precisions ')
        knn_metrics = metrics.create_group('all-k-precisions')
        queries = run_distances.shape[0]
        ks = run_distances.shape[1]

        precisions = np.zeros((ks,queries))
        dataset_distances_arr = np.array(dataset_distances)
        for k in range(ks):
            t = dataset_distances_arr[:,at - 1] + epsilon
            true_positive = np.count_nonzero(run_distances[:,:k+1] <= t[:,None], axis=-1)
            precisions[k] = true_positive / (k + 1)
        knn_metrics['all-k-precisions'] = precisions.mean(-1)
    else:
        print("Found cached result")
    
    return metrics['all-k-precisions']
...
all_metrics = {
    ...
    "all-k-recalls": {
        "description": "Recall@10 for all Ks",
        "function": lambda true_distances, run_distances, metrics, times, run_attrs: get_all_k_recalls_values(true_distances, run_distances, 10, metrics)['all-k-recalls'][:],  # noqa
        "worst": float("-inf"),
        "lim": [0.0, 1.03]
    },
    "all-k-precisions": {
        "description": "Precisions@10 for all Ks",
        "function": lambda true_distances, run_distances, metrics, times, run_attrs: get_all_k_precisions_values(true_distances, run_distances, 10, metrics)['all-k-precisions'][:],  # noqa
        "worst": float("-inf"),
        "lim": [0.0, 1.03]
    },
    "epsilon": {
